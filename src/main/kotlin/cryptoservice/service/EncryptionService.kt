...
class EncryptionService {
    ...
    private fun extractGcpKmsResourceId(config: String): String {
        ...
        val gcpKmsConfig = sopsConfig.keyGroups?.firstOrNull { it.gcpKms?.isNotEmpty() == true }
                    ...
                    when (waitFor()) {
                        EXECUTION_STATUS_OK -> result
                            ...
                            throw SopsEncryptionException(
                                message = "Failed when encrypting RiSc with ID: $riScId by running sops command: sops --encrypt --gcp-kms '$gcpKmsResourceId' --input-type json --output-type yaml /dev/stdin with error message: $result",
                                riScId = riScId,
                            )
                        }
                    }
